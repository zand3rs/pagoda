...
function downloadBookmark(bookmark) {
    ...
    chrome.extension.sendMessage(request, function(response) {
        ...
        if (status == 'success') {
            showBookmarks();
            setFlash(bookmark.title + ' has been successfully popped!');
    ...
    getBookmarks(function(bookmarks) {
        for (var i in bookmarks) {
            ...
            if (bookmark_id == bookmark.id) {
                if (bookmark.archive) {
                    var msg = "Pop " + bookmark.title + " to your hard drive?";
                    if (confirm(msg)) {
...
function showBookmarks() {
    ...
    function _show(bookmark, url, status) {
        ...
        var bookmark_title = bookmark.title || bookmark.url;
        var html = "<div><a id='" + bookmark.id + "' href='" + url + "' " + target + ">" + bookmark_title + "</a></div>";
        var callback = function() {
            switch (status) {
                case 'success':
                    ...
                    break;
                case 'ongoing':
                    setFlash('Please wait while' + bookmark_title + ' is being downloaded to your hard drive.');
                    break;
                case 'error':
                case 'not_found':
                default:
